...
```{r}
#Summary Statistics
tab_death <- df %>% dplyr::select(!record_id,-center) %>%
...
tab_trach <- df %>% dplyr::select(!record_id,-center) %>%
  tbl_summary(digits = list(everything() ~ c(2)),
              statistic = list(all_continuous() ~ "{mean} ({sd})"),
              by = Trach,
              missing = "no") %>%
...
tab <- df %>% dplyr::select(center, Death, Trach, bw,weight_today.36, del_method, prenat_ster,ventilation_support_level.36,inspired_oxygen.36, p_delta.36, med_ph.36, inspired_oxygen.36) %>%
  tbl_summary(digits = list(everything() ~ c(2)),
                   statistic = list(all_continuous() ~ "{mean} ({sd})"),
                   by = center,
              missing = "no") %>%
  add_overall() %>% 
  add_n() %>%
  modify_spanning_header( ~ "**Center**") %>%
  bold_labels() %>%
  as_kable_extra(booktabs = TRUE, caption = "Summary Statistics by Center") %>% 
  kableExtra::kable_styling(latex_options = "scale_down")

latex_code <- kable(tab, format = "latex", booktabs = TRUE, caption = "Summary Statistics Grouped by Center") %>%
  kable_styling(latex_options = "scale_down") %>%
  as.character()
...
df_birthmodel <- df[,c(1:13,29)]
df_36wkmodel <- df[,c(1:19,26,29)]
...
xx$new_trach <- as.factor(xx$new_trach)
...
for (i in 1:5) {
  training_sets_birthmodel[[i]]$new_trach <- as.factor(training_sets_birthmodel[[i]]$new_trach)
  mod <- glmer(new_trach ~ blength + ga + prenat_ster + sga +(1 | center),
...
x_new <- x_new %>% dplyr::select(blength,com_prenat_ster,ga, prenat_ster,sga, new_trach,center)
x_new <- cbind(1,x_new)
x_new <- model.matrix(new_trach ~ blength+ ga+ prenat_ster+sga + center, x_new, 
...
for (i in 1:5) {
  mod <- glmer(new_trach ~ birth_hc + bw + inspired_oxygen.36 + med_ph.36 + peep_cm_h2o_modified.36 + prenat_ster + 
...
x_new <- x_new %>% dplyr::select( new_trach, birth_hc, bw, inspired_oxygen.36,  med_ph.36, peep_cm_h2o_modified.36, prenat_ster , 
               ventilation_support_level.36 , weight_today.36, center)
#x_new <- cbind(1,x_new)
x_new <- model.matrix(new_trach ~   birth_hc+ bw+ inspired_oxygen.36+  med_ph.36+peep_cm_h2o_modified.36+prenat_ster +
...
for (i in 1:5) {
  mod <- glmer(new_trach ~ bw+ inspired_oxygen.36 + med_ph.44 + p_delta.36+ 
...
x_new <- x_new %>% dplyr::select( bw, inspired_oxygen.36, med_ph.44 , p_delta.36,              p_delta.44 ,peep_cm_h2o_modified.44,
                 prenat_ster ,ventilation_support_level.36 , ventilation_support_level_modified.44 , weight_today.44 , new_trach, center)
x_new <- model.matrix(new_trach ~  bw+inspired_oxygen.36+ med_ph.44+ p_delta.36+        p_delta.44+ peep_cm_h2o_modified.44+

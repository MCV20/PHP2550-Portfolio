...
framingham_df %>% dplyr::select(-CVD,-SYSBP_UT,-SYSBP_T,-BMI,-DIABP) %>%
...
df_2017 <- df_2017 %>% filter(AGE >= 30 & AGE <=74) #74?
```

```{r}
df_2017 %>% dplyr::select(-SEQN,-BMI,-SYSBP_UT,SYSBP_T) %>%
  tbl_summary(digits = list(everything() ~ c(2)),
                   statistic = list(all_continuous() ~ "{mean} ({sd})"),
                   by = SEX) %>%
  add_overall() %>% 
...
\begin{table}[htbp]
  ...
  \begin{threeparttable}
    \caption{Summary Statistics by Sex (2017)}
    \begin{tabular}{lcc}
      \toprule
      ...
      Variable\_1 & mean (sd) & mean (sd) \\
      Variable\_2 & mean (sd) & mean (sd) \\
      \midrule
      \multicolumn{3}{l}{\textit{Categorical Variables}} \\
      Variable\_3 & mean (sd) & mean (sd) \\
      Variable\_4 & mean (sd) & mean (sd) \\
      \midrule
      \textbf{Overall} & mean (sd) & mean (sd) \\
      \textbf{N} & n & n \\
      \bottomrule
    \end{tabular}
    \begin{tablenotes}
      \item \textit{Note:} Summary statistics for variables by sex. Continuous variables display mean and standard deviation (sd).
    \end{tablenotes}
  \end{threeparttable}
\end{table}
```
...
#IMPUTATION MEN
nhanes_men_out <- mice(nhanes_men %>% select(-CVD),5, pri=F, seed = 1)
df_imp_men <- vector("list",5)    
for (i in 1:5){df_imp_men[[i]] <- rbind(fram_df_men %>% select(-CVD),mice::complete(nhanes_men_out,i))}
...
for (i in 1:5) {
  ...
  #FIT Inverse Odds
  #Do the same:
  inv_odd_men <- glm(as.factor(S) ~ log(HDLC)+log(TOTCHOL)+log(AGE)+log(SYSBP_UT+1)+ log(SYSBP_T+1)+CURSMOKE+DIABETES ,
                       data = df, 
                       family = "binomial")
  prob_fram_men <- predict(inv_odd_men, join_men, type = "response")
  inv_prob_fram_men <- (prob_fram_men/(1-prob_fram_men))^(-1)
  
  nhanes_men_brier[i] <- sum(inv_prob_fram_men[1:length(y_men$CVD)]*(y_men$CVD - pred_prob_men)^2)/nrow(df %>% filter(S==0))
}
brier_nhanes_men <- mean(nhanes_men_brier)
sd_brier_nhanes_men <- sd(nhanes_men_brier)

#IMPUTATION WOMEN
nhanes_women_out <- mice(nhanes_women %>% select(-CVD),5, pri=F, seed = 1)
df_imp_women <- vector("list",5)    
for (i in 1:5){df_imp_women[[i]] <- rbind(fram_df_women %>% select(-CVD),mice::complete(nhanes_women_out,i))}

nhanes_women_brier <- NA
for (i in 1:5) {
  df <- df_imp_women[[i]]
  #FIT Inverse Odds
#Do the same:
inv_odd_women <- glm(as.factor(S) ~ log(HDLC)+log(TOTCHOL)+log(AGE)+log(SYSBP_UT+1)+ log(SYSBP_T+1)+CURSMOKE+DIABETES ,
                     data = df, 
                     family = "binomial")
prob_fram_women <- predict(inv_odd_women, join_women, type = "response")
inv_prob_fram_women <- (prob_fram_women/(1-prob_fram_women))^(-1)

nhanes_women_brier[i] <- sum(inv_prob_fram_women[1:length(y_women$CVD)]*(y_women$CVD - pred_prob_women)^2)/nrow(df %>% filter(S==0))
}
brier_nhanes_women <- mean(nhanes_women_brier)
sd_brier_nhanes_women <- sd(nhanes_women_brier)
...
nhanes_men_brier <- sum(inv_prob_fram_men[1:length(y_men$CVD)]*(y_men$CVD - pred_prob_men)^2)/nrow(nhanes_men)
...
nhanes_women_brier <- sum(inv_prob_fram_women[1:length(y_women$CVD)]*(y_women$CVD - pred_prob_women)^2)/nrow(nhanes_women)
...
sim_data <- mvrnorm(n = 3055, mu = mu, Sigma = cov)
...
sim_data <- mvrnorm(n = 3544, mu = mu, Sigma = cov)
sim_data <- as.data.frame(sim_data)
...
p.cor.med <- ggcorrplot(cor_med) + ggtitle("Moderate Correlation")
p.cor.no <- ggcorrplot(cor_no) + ggtitle("Independent")
...
p <- framingham_df_men_log %>% 
  dplyr::select(-S,-log_SYSBP_T,-log_SYSBP_UT)%>%
  mutate(CURSMOKE = as.numeric(CURSMOKE)-1, 
         DIABETES = as.numeric(DIABETES)-1,
         BPMEDS = as.numeric(BPMEDS)-1) %>%
  cor() %>%
...
ggarrange(p1,p,common.legend = T,legend = "bottom")
...
brier_similar_men[i] <- sum(inv_prob_fram_men[1:length(y_men$CVD)]*(y_men$CVD - pred_prob_men)^2)/nrow(sim_data_similar_men)
print(i)
}
...
brier_high_men[i] <- sum(inv_prob_fram_men[1:length(y_men$CVD)]*(y_men$CVD - pred_prob_men )^2)/nrow(sim_data_high_men)
print(i)
}
...
brier_med_men[i] <- sum(inv_prob_fram_men[1:length(y_men$CVD)]*(y_men$CVD - pred_prob_men)^2)/nrow(sim_data_med_men)
print(i)
}
...
brier_nocor_men[i] <- sum(inv_prob_fram_men[1:length(y_men$CVD)]*(y_men$CVD - pred_prob_men)^2)/nrow(sim_data_nocor_men)
print(i)
}
...
pbrier_nocor_men <- ggplot(as.data.frame(brier_nocor_men),aes(x = brier_nocor_men))+
 ...
 c(brier_nhanes_men,brier_similar_men_avg,brier_med_men_avg,brier_high_men_avg,brier_nocor_men_avg)
...
p <- framingham_df_women_log %>% 
  dplyr::select(-S,-log_SYSBP_UT,-log_SYSBP_T)%>%
  mutate(CURSMOKE = as.numeric(CURSMOKE)-1, 
         DIABETES = as.numeric(DIABETES)-1,
         BPMEDS = as.numeric(BPMEDS)-1) %>%
  cor() %>%
...
inv_odd_women <- glm(as.factor(S) ~ log_HDLC + log_TOTCHOL + log_AGE +
  ...
  brier_similar_women[i] <- sum(inv_prob_fram_women[1:length(y_women$CVD)]*(y_women$CVD - pred_prob_women)^2)/nrow(sim_data_similar_women)
  print(i)
...
for(i in 1:5000){
  ...
  brier_high_women[i] <- sum(inv_prob_fram_women[1:length(y_women$CVD)]*(y_women$CVD -pred_prob_women)^2)/nrow(sim_data_high_women)
  print(i)
}
...
for(i in 1:5000){
  ...
  brier_med_women[i] <- sum(inv_prob_fram_women[1:length(y_women$CVD)]*(y_women$CVD - pred_prob_women)^2)/nrow(sim_data_med_women)
  print(i)
}
...
for(i in 1:5000){
  ...
  brier_nocor_women[i] <- sum(inv_prob_fram_women[1:length(y_women$CVD)]*(y_women$CVD - pred_prob_women)^2)/nrow(sim_data_nocor_women)
  print(i)
}
...
tab_men <- res_men %>% 
  ...
  kableExtra::kable_styling(font_size = 8, latex_options = "HOLD_position") %>%     add_footnote(paste("NHANES Brier score is", round(brier_nhanes_men,4)))
...
tab_women <- res_women %>% 
  ...
  kableExtra::kable_styling(font_size = 8, latex_options = "HOLD_position") %>%     add_footnote(paste("NHANES Brier score is", round(brier_nhanes_women,4)))
...
#bias
c(brier_nocor_women_avg, brier_similar_women_avg, brier_med_women_avg, brier_high_women_avg) - brier_nhanes_women

c(brier_nocor_men_avg, brier_similar_men_avg, brier_med_men_avg, brier_high_men_avg)-brier_nhanes_men
